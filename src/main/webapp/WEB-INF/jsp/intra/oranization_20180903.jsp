...
<script type="text/javascript">
	...
	function treePopup(result, mildsc, nodeId) {
		...
		if (result.list.length != 0) {
			...
			//$("#tPop").hide();
			//$("#datatext").show();
			html = "<th><td colspan='6'>조직정보가 없습니다.</td><th>";
			$("#tPop").html(html);
		}
		//$("#popWrap").dialog("open");
	}
	...
	function setTreeFacilltyPaging(result,mildsc,nodeId){
		...
		for (var i = result.paging.startPageNo; i <= result.paging.endPageNo; i++) {
			if (i == result.paging.pageNo) {
				if(result.paging.pageNo != 0) {
					htmlStr += "<a href='javascript:void(0)' id='" + i + ","+mildsc +","+nodeId+"' class='num'><b>" + i + "</b></a>";
				} else {
					htmlStr += "<a href='javascript:void(0)' id='" + 1 + ","+mildsc +","+nodeId+"' class='num'><b>" + 1 + "</b></a>";
				}
				...
				if(result.paging.pageNo != 0) {
					...
					htmlStr += "<a href='javascript:void(0)' id='" + 1 + ","+mildsc +","+nodeId+"' class='num'>" + 1 + "</a>";
				}
...
<body onload="resizeTo(880,700)">
				...
				<div class="custom_basic">
					<div id="right-fixedHeader" class="section">
						...
						<!-- <div id="tableContainer" class="container-tb" style="border-left: 1px solid #e2e1e9; border-right: 1px solid #e2e1e9;"> -->
							<!-- <div id="tableContainer" class="inner-wrap t2 " style="border-left: 1px solid #e2e1e9; border-right: 1px solid #e2e1e9;"> -->
							<table id="oranization-table" class="main-table customers_t container-tb">
								<thead>
									<tr>
	...
	<script>
		...
		function scroll() {
			// 1번 스크롤
		    var fauxTable1 = document.getElementById("t3");
		    var mainTable = document.getElementById("oranization-table");
