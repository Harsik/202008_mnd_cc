...
<%@ include file="/WEB-INF/jsp/layout/include/incHeader.jspf" %>
	...
	<script type="text/javascript">
		...
		/* $(document).on("click", "#subBt", function() {
			location.href= "/admin/adminModify.do";
		});	 */	
		
		function adminDetail(seq) {
			var frm = document.form1;
			frm.seq.value = seq;
			frm.action = "/admin/adminModify.do";
			frm.submit();
		};
		...
		<div class="content_main">
			...
			<div class="contents_a">
						...
						<fieldset>
							<legend>검색</legend>
								<select name="searchKey" id="searchKey" title="조건을 선택하세요" class="select-type">
									...
									<%-- <option value="2" ${paramMap.searchKey.equals('2')?'selected':'' }>권한정보</option> --%>
								</select>
				...
				<div class="board_type_n">	
					<table class="tbp_type_board" border="1" cellspacing="0" summary="최상위관리자 목록 게시판입니다.">
						<caption>최상위관리자 목록 게시판</caption>
						<colgroup>
						<col width="7%">
						<col>
						<col>
						...
						<tbody>
							...
							<tr>
								...
								<td id="subBt"><a href="javascript:{adminDetail('<c:out value="${data.seq}"/>')}"  ><c:out value="${data.mngrId}"/></a></td>
								<c:if test="${data.auth == 0}">
									<td class="center">시스템관리자</td>
								</c:if>
				...
				<div class="tbl_bottom">
					<div class="t_right">
					  <button type="button" name="addBt" id="addBt" class="btnComm grblue" title="등록">등록</button>
					</div>
				</div>
				<!--//btn_area-->
			</div>
